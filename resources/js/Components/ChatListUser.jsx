...
export default function ChatListUser() {
    const { props } = usePage();
    let {chat_with: chatWithUser, auth, ziggy} = props;
    ...
    useEffect(() => {
        users.forEach(user => {
            ...
            if (receiveMessage && sendMessage) setSelectedChat(prev => (receiveMessage?.id > sendMessage?.id ? receiveMessage : sendMessage));
            else if (receiveMessage) setSelectedChat(prev => receiveMessage);
            else if (sendMessage) setSelectedChat(prev => sendMessage);
        });
    }, [users]);
