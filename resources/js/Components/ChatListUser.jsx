...
export default function ChatListUser({ settings }) {
            ...
            <div className="flex-1 mt-3 overflow-y-auto" scroll-region="true">
                {
                    users.map((user) => {
                        ...
                        return <Link preserveScroll key={user.uuid} href={route('chat.show', user.uuid)}
                                     className={'bg-transparent flex w-full items-center hover:bg-gray-800/60 px-2.5 py-3 rounded-md'}>
                            <div className="items-center mr-3 flex-2">
                                <ProfilePictureOnChat entity={user}/>
                            </div>
                            <div className="flex flex-col flex-1 min-w-0 pr-2">
                                <div className="flex items-center justify-between">
                                    <div style={{color: settings.sidebar_text_color}} className="text-gray-100 text-sm font-medium truncate mb-1.5">
                                        {user.name}
                                    </div>
                                    <div style={{color: settings.sidebar_text_color}} className="text-[10px] text-gray-400 mb-1">
                                        {chat?.sent_at}
                                    </div>
                                </div>
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center text-xs text-gray-400">
                                        ...
                                        {chat?.message_deleted_at
                                            ? <span style={{color: settings.sidebar_text_color}} className="mr-2 italic text-gray-500">{chat?.message}</span>
                                            : <div className="overflow-hidden break-all"  style={{ color: settings.sidebar_text_color, display: "-webkit-box", WebkitLineClamp: 1, WebkitBoxOrient: "vertical" }}>{chat?.message}</div>
                                        }
                                    </div>
                                    {user.messages_count > 0 && (
                                        <div className="inline-flex items-center px-1.5 rounded-full text-[10px] bg-purple-500 text-white" style={{color: settings.sidebar_text_color}}>
                                            {user.messages_count}
                                        </div>
                                    )}
                                </div>
